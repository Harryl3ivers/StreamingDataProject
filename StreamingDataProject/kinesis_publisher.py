...
class KinesisPublisher:
    ...
    def publish_articles(self, articles):
        ...
        logger.info(
            f"Published {len(articles)} articles to Kinesis stream '{self.stream_name}'"
        )
    ...
    def _ensure_stream_exists(self):
        ...
        try:
            ...
            if status != "ACTIVE":
                logger.info(
                    f"Waiting for stream '{self.stream_name}' to become ACTIVE..."
                )
            ...
            current_retention = response["StreamDescription"].get(
                "RetentionPeriodHours", 24
            )
            if current_retention < 72:
                logger.info(
                    f"Updating stream '{self.stream_name}' retention to 3 days..."
                )
            ...
            if e.response["Error"]["Code"] == "ResourceNotFoundException":
                ...
                logger.info(
                    f"Setting stream '{self.stream_name}' retention to 3 days..."
                )
                self.kinesis.increase_stream_retention_period(
                    StreamName=self.stream_name, RetentionPeriodHours=72
                )
